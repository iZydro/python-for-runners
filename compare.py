...
def read_results(file_name, vector_name):
    for line in open(file_name, "r"):
        ...
        if first.isdigit():
            ...
            name = runner_time[0]
            time = runner_time[1]
        name = name.replace(chr(0xa0), " ")
        name = name.replace(chr(0xc2), "")
        ...
        vector_name.append([name, time])

def divide_time(my_time, total):
    result = {}

    seconds = (my_time - datetime.strptime("0:00:00", "%H:%M:%S")).total_seconds()
    #seconds = my_time.second + my_time.minute * 60 + my_time.hour * 3600 + (my_time.day - 1) * 3600 * 24

    div = int(seconds / total)
    hours = int(div/3600)
...
cnt_race = 0
total_race = init_diff
for race in bcn:
    time_race = datetime.strptime(race[1], "%H:%M:%S")
    total_race += timedelta(0, time_race.hour * 3600 + time_race.minute * 60 + time_race.second)
    cnt_race += 1
result = divide_time(total_race, cnt_race)
